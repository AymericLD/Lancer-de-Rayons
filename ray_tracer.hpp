...
class Pixel
{
    ...
    public :
    Pixel();
    rayon rayon_pixel(vector<float> direction)
    {
        rayon r();
        r.origine = centre;
...
class Grille
{
    ...
    Grille(int l,int h, float t);
};

Grille::Grille(int l, int h, float t) // Constructeur de la classe grille, on initialise une grille de dimension donnée avec des pixels ayant (0,0,0 pour valeur rgb)
